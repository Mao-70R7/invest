...
- 策略详情：1137 个。
- 策略源数据完整标记：1107 个；源数据不完整：30 个，页面保留真实缺失状态，不做推测补齐。
- 基金详情：17601 个，覆盖全市场排名、策略当前持仓及历史持仓下钻。
- 仅基础详情：0 个；对应源站没有增强详情时仍可展示基础信息，不伪造净值或基准。
- 策略对比仓位快照：只保留每只策略最新有效快照，原始 716253 行，发布 74250 行；不影响当前配置对比和 AI 持仓筛选。
- 调仓月报：发布静态报告 `basic_data/monthly-rebalance-report-202606.html`，图片资产 8 个，合计 668842 字节。
